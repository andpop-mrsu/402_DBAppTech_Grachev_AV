# Проект "Сапер" с REST API на Slim Framework (Minesweeper SPA)

Полнофункциональное одностраничное веб-приложение классической игры "Сапёр" (Minesweeper) с серверным бэкендом на PHP с использованием микрофреймворка **Slim Framework** и базой данных SQLite. Игрок открывает ячейки на прямоугольном поле, избегая мин. Цель игры — открыть все безопасные клетки. Если игрок открывает ячейку с миной, игра заканчивается поражением.

Данная версия является миграцией Task08 на современный микрофреймворк Slim, обеспечивая:

- **Стандартизированная архитектура** — использование PSR-7 стандарта для HTTP-сообщений
- **Slim Framework** — проверенное решение для маршрутизации и обработки запросов
- **Улучшенная обработка ошибок** — централизованная система через ErrorMiddleware
- **100% совместимость с Task08** — идентичное API и формат ответов
...
## Архитектура на Slim Framework

### Преимущества миграции на Slim

1. **Стандартизация** — использование PSR-7 стандарта для HTTP-сообщений
2. **Простота** — декларативное определение маршрутов
3. **Надежность** — проверенное решение с активным сообществом
4. **Расширяемость** — поддержка middleware для дополнительной функциональности
5. **Улучшенная обработка ошибок** — централизованная система через ErrorMiddleware
6. **Тестируемость** — легко покрывается автоматическими тестами
...
- **Composer** — менеджер зависимостей для установки Slim Framework
...
2. Установите зависимости через Composer:
   ...
   composer install
   ```
   Эта команда установит Slim Framework и все необходимые зависимости.
...
5. База данных будет создана автоматически при первом запросе к API.
...
- Slim Framework и зависимости устанавливаются через Composer
- Для production рекомендуется настроить `displayErrorDetails: false` в ErrorMiddleware
